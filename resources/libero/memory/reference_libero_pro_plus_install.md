...
description: Reproduce LIBERO-Pro + LIBERO-Plus install on the existing openpi venv via resources/libero/install_libero_pro_plus.sh; HF snapshot at /mnt/public2/zhangyixian/datasets/liberopro_hf/
metadata: 
  node_type: memory
  type: reference
...
**Install script**: `examples/embodiment/primitives/resources/libero/install_libero_pro_plus.sh`

Re-runs LIBERO-Pro + LIBERO-Plus install on top of the **existing** openpi venv (no fresh venv, no torch reinstall). Idempotent:

```bash
bash examples/embodiment/primitives/resources/libero/install_libero_pro_plus.sh           # both
bash examples/embodiment/primitives/resources/libero/install_libero_pro_plus.sh --only pro
bash examples/embodiment/primitives/resources/libero/install_libero_pro_plus.sh --only plus
```
