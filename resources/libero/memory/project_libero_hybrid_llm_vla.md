...
description: "Hybrid LLM-in-the-loop + Pi0.5 VLA pipeline solved libero_spatial task 0 (On(bowl,plate)); driver at examples/embodiment/primitives/"
metadata: 
  node_type: memory
  type: project
...
- Code: `examples/embodiment/primitives/{test_hybrid_all_spatial.py, test_hybrid_failed_retry.py}`. Per-rollout JSONs in `results_all_spatial{,_retry}/`.
...
- Code: `examples/embodiment/primitives/interactive_driver.py` extended with `--suite libero_10 --task <N>` flags and `reset` REPL command.
...
- Interactive driver: `examples/embodiment/primitives/interactive_driver.py` — REPL via `/tmp/hybrid_repl/command.json` polling; supports move_to/pi0_pick/release/set_gripper/reset/exit actions; dumps state_<N>.json + image_<N>.png after every command.
