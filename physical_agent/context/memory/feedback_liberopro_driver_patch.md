...
In `physical_agent/backends/rlinf/repl_driver.py`, `make_env` originally did `from libero.libero.benchmark import get_benchmark`. That registry has no PRO suites — `get_benchmark('libero_spatial_task')` raises `KeyError` even with `LIBERO_TYPE=pro`.
