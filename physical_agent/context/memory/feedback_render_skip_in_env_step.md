...
6. `physical_agent/backends/rlinf/repl_driver.py` `execute()`: pre-primitive toggle (OSC primitives → disable, pi0_pick → enable). No post-primitive refresh step (robosuite's first sample after re-enable returns degenerate (1,1,3) float64).
