...
**What it does:** new primitive in `examples/embodiment/primitives/primitives.py:rotate_pitch`. Tilts the gripper around the world X axis, holding xyz/yaw constant. Wired into `interactive_driver.py` as `{"action": "rotate_pitch", "delta_pitch": ..., "gripper": ...}`.
