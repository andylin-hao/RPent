...
1. **Best**: read the sister-cell audit (task/swap/lan version of same task) in `resources/libero/results_*/`'s `final_state.objects` — that's the ground-truth world coord where the predicate fired.
